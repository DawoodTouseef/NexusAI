...
def get_model(repo_id,MODEL,local_dir="hf_cache"):
    ...
    for model in models.siblings:
        if model.rfilename=="README.md":
            ...
            with open("/media/lenovo/Windows 10/NexusAI/backend/NexusAI/Server/data/p1_models.jsonl","w") as f:
                for i in MODEL:
...
if __name__=="__main__":
    MODELS = [json.loads(line) for line in open("/media/lenovo/Windows 10/NexusAI/backend/NexusAI/Server/data/p0_models.jsonl", "r").readlines()]
    get_model("meta-llama/Meta-Llama-3.1-405B-Instruct",MODELS)